...
class BatchHybridMonteCarloExpectedImprovement(SingleModelAcquisitionBuilder[HasReparamSampler]):
    """
    Expected improvement for batches of points (or :math:`q`-EI), approximated using Monte Carlo
    estimation with the reparametrization trick. See :cite:`Ginsbourger2010` for details.
    Improvement is measured with respect to the minimum predictive mean at observed query points.
    ...
    def __repr__(self) -> str:
        """"""
        return f"BatchHybridMonteCarloExpectedImprovement({self._sample_size!r}, jitter={self._jitter!r})"
    ...
    def prepare_acquisition_function(
        ...
        eta = tf.reduce_min(mean, axis=0)
        return batch_hybrid_monte_carlo_expected_improvement(self._sample_size, model, eta, self._jitter)
    ...
    def update_acquisition_function(
        ...
        tf.debugging.assert_positive(len(dataset), message="Dataset must be populated.")
        tf.debugging.Assert(
            isinstance(function, batch_hybrid_monte_carlo_expected_improvement), [tf.constant([])]
        )
        mean, _ = model.predict(dataset.query_points)
        eta = tf.reduce_min(mean, axis=0)
        function.update(eta)  # type: ignore
...
class batch_hybrid_monte_carlo_expected_improvement(AcquisitionFunctionClass):
    def __init__(self, sample_size: int, model: HasReparamSampler, eta: TensorType, jitter: float):
        """
        :param sample_size: The number of Monte-Carlo samples.
        :param model: The model of the objective function.
    ...
    def __call__(self, x: TensorType) -> TensorType:
        ...
        # Draw samples and make diagonal predictions
        samples = tf.squeeze(self._sampler.sample(x, jitter=self._jitter), axis=-1)  # [..., S, B]
        ...
        min_sample_per_batch = tf.reduce_min(samples, axis=-1)  # [..., S]
        batch_improvement = tf.maximum(self._eta - min_sample_per_batch, 0.0)  # [..., S]
        batch_expected_improvement = tf.reduce_mean(batch_improvement, axis=-1, keepdims=True)  # [..., 1]

        # Compute analytic pointwise EI
        normal = tfp.distributions.Normal(mean, tf.sqrt(variance))
        ...
        print(f"mean_pointwise_ei.shape, {mean_pointwise_ei.shape}")

        # Compute pointwise MCEI
        ...
        print(f"expected_mean_pointwise_improvements.shape, {expected_mean_pointwise_improvements.shape}")

        # Compute analytic point-to-point EI
        idx_grid = tf.stack(tf.meshgrid(tf.range(x.shape[-1]), tf.range(x.shape[-1])), axis=-1)  # [B, B, 2]
        idx = tf.reshape(idx_grid, (-1, 2))

        # Compute Monte Carlo point-to-point EI
